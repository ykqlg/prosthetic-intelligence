...
int get_positive_number()
{
    int isValid = 0, userInput;
    while (!isValid)
    {
        printf("Input sample number (positive integer): ");
...
void loop(pSensor arg)
{
    ...
    snprintf(outputFileName, sizeof(outputFileName), "Sensor%d_%s.txt", arg->sensorIndex, formattedTime);
    // 打开要写入的文件
