...
void getFilePath(pSensor arg)
{
    ...
    snprintf(outputFileName, sizeof(outputFileName), "%s/Sensor%d_%s.csv",dataFolderPath, 1, arg->startTime);
...
void loop(pSensor arg)
{
    ...
    // 使用格式化字符串作为文件名
    char outputFileName[100];
    snprintf(outputFileName, sizeof(outputFileName), "%s/Sensor%d_%s.csv", dataFolderPath, arg->sensorIndex, arg->startTime);
